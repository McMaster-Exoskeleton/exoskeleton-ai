"""
Training script for single joint TCN model on exoskeleton joint moment estimation.

Usage:
    # Basic training with default config
    python scripts/train_single_joint_tcn.py

    # Override specific parameters
    python scripts/train_single_joint_tcn.py training.num_epochs=50 training.batch_size=16

    # Use different model variant
    python scripts/train_single_joint_tcn.py model=tcn_small

    # Hyperparameter sweep
    python scripts/train_single_joint_tcn.py -m model.architecture.num_channels=[15,15,15],[25,25,25,25,25]
"""
...
def masked_loss(
    ...
    Args:
        outputs: Model predictions (batch, seq_len, 1).
...
def train_epoch(
    ...
    for batch_idx, batch in enumerate(progress_bar):
        ...
        inputs = batch["inputs"][:, :, feature_idx].to(device)  # (batch, seq_len, 7)
        targets = batch["targets"][:, :, joint_idx:joint_idx+1].to(device)  # (batch, seq_len, 1)
...
def validate(
    ...
    for batch in progress_bar:
        inputs = batch["inputs"][:, :, feature_idx].to(device)
        targets = batch["targets"][:, :, joint_idx:joint_idx+1].to(device)
    ...
    all_outputs = torch.cat(all_outputs, dim=0)  # (total_valid_timesteps, 1)
    all_targets = torch.cat(all_targets, dim=0)  # (total_valid_timesteps, 1)
    ...
    all_outputs = all_outputs.unsqueeze(0)  # (1, total_timesteps, 1)
...
def train(cfg: DictConfig) -> None:
    ...
    test_loss, test_metrics = validate(
        model, test_loader, criterion, device, 0, cfg)
